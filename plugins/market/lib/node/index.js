...
var Installer = class extends import_koishi.Service {
  ...
  async _getGitPackage(name2, path) {
    try {
      ...
      logger.warn(`Cannot get git package ${name2} with ${path}: ${e.message}`);
      this.pkgTasks[name2] = void 0;
    }
  }
  ...
  async install(deps, forced) {
    ...
    deps = (0, import_koishi.mapValues)(deps, (request, name2) => Object.hasOwn(localDeps, name2) && (0, import_semver.valid)(request) ? Dependency.stringify(localDeps[name2], request) : request);
    ...
    for (const key in localDeps) {
      ...
      await this.ctx.parallel("shutdown");
