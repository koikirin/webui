...
async function apply(ctx, config) {
  ...
  ctx.inject(["console"], (ctx2) => {
    const entry = ctx2.console.addEntry(process.env.KOISHI_BASE ? [
    ...
    ctx2.console.addListener("l10n", async (data) => {
      for (const locale in data) {
        ctx2.i18n.define("$" + locale, data[locale]);
        const content = (0, import_js_yaml.dump)(data[locale]);
        await (0, import_promises.writeFile)((0, import_path.resolve)(ctx2.baseDir, config.root[0], locale + ".yml"), content);
      }
    }, { authority: 4 });
    const update = ctx2.debounce(() => entry.refresh(), 0);
    ctx2.on("internal/i18n", update);
  });
}
__name(apply, "apply");
