...
var Insight = class extends import_console.DataService {
  static {
    __name(this, "Insight");
  }
  constructor(ctx) {
  ...
  async get() {
    ...
    const edges = [];
    const services = {};
    for (const [key, { type }] of Object.entries(this.ctx.root[import_koishi.Context.internal])) {
      if (type !== "service") continue;
      const instance = this.ctx.get(key);
      if (!(instance instanceof Object)) continue;
      const ctx = Reflect.getOwnPropertyDescriptor(instance, import_koishi.Context.current)?.value;
      if (ctx?.scope.uid) {
